...
```text
SHTUCodeProxy-v4.2.12-windows-x64.exe
```
...
```text
SHTUCodeProxy-v4.2.12-windows-x64.zip
```
...
```text
SHTUCodeProxy-v4.2.12-linux-x86_64
SHTUCodeProxy-v4.2.12-linux-x86_64-python-launcher.tar.xz
shtucodeproxyctl-v4.2.12-linux-x86_64
```

For the single-file binary, make it executable and run it from a desktop session:

```bash
chmod +x SHTUCodeProxy-v4.2.12-linux-x86_64
./SHTUCodeProxy-v4.2.12-linux-x86_64
```
...
```bash
tar -xf SHTUCodeProxy-v4.2.12-linux-x86_64-python-launcher.tar.xz
cd SHTUCodeProxy-v4.2.12-linux-x86_64-python-launcher
...
- On headless servers, use `shtucodeproxyctl-v4.2.12-linux-x86_64` or source CLI mode instead of the GUI.
...
```text
release\SHTUCodeProxy-v4.2.12-windows-x64.exe
release\SHTUCodeProxy-v4.2.12-windows-x64.zip
dist\SHTUCodeProxy\SHTUCodeProxy.exe
```

Use `release\SHTUCodeProxy-v4.2.12-windows-x64.exe` for normal Windows users. It is a single-file executable and does not require Python or the `_internal` folder.

Use `release\SHTUCodeProxy-v4.2.12-windows-x64.zip` as the Windows portable folder package. If you distribute the zip package, users must extract the whole folder because the `_internal` runtime folder is required by the folder build.
...
```bash
unzip SHTUCodeProxy-v4.2.12-source-linux-macos.zip
cd SHTUCodeProxy-v4.2.12-source
...
```bash
chmod +x shtucodeproxyctl-v4.2.12-linux-x86_64
cp headless-config.example.json config.json
nano config.json
./shtucodeproxyctl-v4.2.12-linux-x86_64 apply-config config.json --write-claude --write-codex --start
./shtucodeproxyctl-v4.2.12-linux-x86_64 status
```
